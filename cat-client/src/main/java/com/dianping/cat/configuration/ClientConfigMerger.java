...
public class ClientConfigMerger extends DefaultMerger {
    ...
    protected void mergeDomain(Domain old, Domain domain) {
        ...
        LogUtil.info("合并 Domain ", U.format("old", U.toJSS(old), "domain", U.toJSS(domain)));
        ...
        LogUtil.info("Domain合并结果", U.format("old", U.toJSS(old), "domain", U.toJSS(domain)));
    ...
    protected void visitConfigChildren(ClientConfig to, ClientConfig from) {
        ...
        LogUtil.info("合并 ClientConfig", U.format("to", U.toJSS(to), "from", U.toJSS(from)));
        ...
        if (to != null) {
            ...
            LogUtil.info("ClientConfig合并结果", U.format("to", U.toJSS(to), "from", U.toJSS(from)));
