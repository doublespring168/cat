...
<table id="web_content" class="table table-striped table-condensed">
    ...
    <c:choose>
        <c:when test="${payload.sort eq 'request' }">
            <c:forEach var="item" items="${model.commandDisplayInfo.distributeDetails.requestSortedItems}"
                       varStatus="status">
                <tr>
                    <c:if test="${payload.groupByField.name eq 'code'}">
                        <c:choose>
                            <c:when test="${model.codes[item.id] != null}">
                                ...
                                <td><a class="btn btn-xs"
                                       href="/cat/s/config?op=appCodeUpdate&id=${model.commandId}&code=${item.id}">编辑</a>
                                </td>
                            </c:when>
                            <c:otherwise>
                                <td>${item.id}</td>
        ...
        <c:otherwise>
            <c:forEach var="item" items="${model.commandDisplayInfo.distributeDetails.delaySortedItems}"
                       varStatus="status">
                <tr>
                    <c:if test="${payload.groupByField.name eq 'code'}">
                        <c:choose>
